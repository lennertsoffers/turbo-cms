...
import {
    createPageAction,
    CreatePageActionState
} from "./actions/create-page.action";
import { PageCreator } from "./components/page-creator/page-creator";

type PageCreatorPaneProps = PaneProps<{}>;

export const PageCreatorPane = ({
    paneId,
    paneState
}: PageCreatorPaneProps) => {
    const { t } = useTranslation<BaseTranslations, TranslationKey>();

    const action = createPageAction;

    return (
        <Pane<CreatePageActionState>
            initialState={{
            ...
            action={action}
            cancelButtonText={t("custom:window:pane:page_creator:footer:cancel")}
            pane={PaneEnum.PAGE_CREATOR}
            paneId={paneId}
            paneState={paneState}
            resetOnClose={true}
            submitButtonText={t("custom:window:pane:page_creator:footer:save")}
